...
local function get_base()
	...
	return {
  	...
  	CursorIM = { fg = cp.black2, bg = cp.white }, -- like Cursor, but used when in IME mode |CursorIM|
		...
		SpellBad = { fg = cp.red, style = "undercurl" }, -- Word that is not recognized by the spellchecker. |spell| Combined with the highlighting used otherwise.
		SpellCap = { fg = cp.yellow, style = "undercurl" }, -- Word that should start with a capital. |spell| Combined with the highlighting used otherwise.
		SpellLocal = { fg = cp.blue, style = "undercurl" }, -- Word that is recognized by the spellchecker as one that is used in another region. |spell| Combined with the highlighting used otherwise.
		SpellRare = { fg = cp.green, style = "undercurl" }, -- Word that is recognized by the spellchecker as one that is hardly ever used.  |spell| Combined with the highlighting used otherwise.
		...
		-- diff
		diffAdded = { bg = cp.green, fg = cp.black2 },
		diffRemoved = { bg = cp.red, fg = cp.black2 },
		...
		DiffAdd = { bg = cp.green, fg = cp.black2 }, -- diff mode: Added line |diff.txt|
		DiffChange = { bg = cp.yellow, fg = cp.black2 }, -- diff mode: Changed line |diff.txt|
		DiffDelete = { bg = cp.red, fg = cp.black2 }, -- diff mode: Deleted line |diff.txt|
		DiffText = { bg = cp.blue, fg = cp.black2 }, -- diff mode: Changed text within a changed line |diff.txt|
		-- NeoVim
